//! E2E test bridge: a debug-only channel between the running app and
//! external test drivers (the native headless harness + Playwright in a
//! headless browser).
//!
//! Compile gating: active in `cargo test`, in debug builds
//! (`debug_assertions`) — which includes the trunk debug bundle the
//! browser E2E suite runs against — and compiled OUT of release builds,
//! so shipped binaries carry zero overhead.
//!
//! Three data channels, refreshed every frame in [`ForgeApp::ui_body`]:
...
//!   pushes requests that [`ForgeApp::ui_body`] drains next frame —
//!   this drives features that need file bytes (imports) without a
//!   native file dialog.
...
// egui calls — the record is the only addition.
// ---------------------------------------------------------------------------
